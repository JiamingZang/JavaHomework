      ...
      >
        ...
        >
          <template #default="scope">
            <el-button size="mini" @click="getStudents(scope.row.id)"
              >查看
              <el-icon class="el-icon--right">
                <EditPen />
              </el-icon>
        ...
        >
          <template #default="scope">
            <el-popconfirm
              title="确定选课吗?"
              @confirm="addStudentcourse(scope.row.id)"
            >
              <template #reference>
...
export default {
  ...
  methods: {
    ...
    getStudents(cid) {
      request
        .get("/studentcourse/getAllStudentsByCid", {
          params: {
            cid: cid,
          },
        })
    ...
    addStudentcourse(cid) {
      var param = {
        cid: cid,
        sid: JSON.parse(localStorage.getItem("user"))["id"],
      };
      request.post("/studentcourse/addStudentcourse", param).then((res) => {
        if (res.code === 200) {
